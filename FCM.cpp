...
void FCM::_computeCenters(){
    ...
    for(int j = 0; j < _clusterNum; ++j) {
        ...
        for(int i = 0; i < _dataMatrix->rows(); ++i) {
            numerator += t(i, j) * (*_dataMatrix)(i, Eigen::all);
        }
        double denominator = (*_membershipMatrix)(Eigen::all, j).sum();

        (*_clusterCenters)(j, Eigen::all) = numerator / denominator;
    }
...
double FCM::_calculateDistance(unsigned int i, unsigned int k){
  ...
  */
  ...
  return ((*_dataMatrix)(k, Eigen::all) - (*_clusterCenters)(i, Eigen::all)).lpNorm<2>();
}
